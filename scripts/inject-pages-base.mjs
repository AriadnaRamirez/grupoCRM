...
const cssBoot = `  <style id="css-boot">
  /* Critical first paint: chrome + hero before main.css (FOUC / CLS) */
  ...
  .slide__photo.media-load:not(.is-ready) { background-image: none !important; }
...
const cssBootBlock =
  /<style id="css-boot">[\s\S]*?<\/style>\s*(?:<noscript><style>html \{ visibility: visible !important; \}<\/style><\/noscript>\s*)?(?:<script>setTimeout\(function \(\) \{ document\.documentElement\.classList\.add\("is-booted"\); \}, 4000\);<\/script>\s*)?/g;
const block = /<script>\s*\/\* pages-base \*\/[\s\S]*?<\/script>\s*/g;
const externalBlock =
  /<script(?: src="\/js\/pages-base\.js"><\/script>|[^>]*>!function\(\)\{if\(!\/\\.github\\.io\$\/i\.test\(location\.hostname\)\)return;[\s\S]*?<\/script>)\s*/g;
...
for (const file of walk(root)) {
  ...
  if (!html.includes(snippet.trim())) {
    html = html.replace(/(<meta charset="UTF-8">\s*\r?\n)/i, `$1${snippet}`);
  }
  if (/css\/(?:tokens|main)(?:\.min)?\.css/.test(html) && !html.includes('id="css-boot"')) {
    html = html.replace(
...
console.log(`injected pages-base + css-boot into ${changed} html files`);
